...
class StoreQueueInterface(parameter: StoreQueueParameter) extends Bundle {
  ...
  val wb = Flipped(ValidIO(new SqWb(parameter.sqSize, parameter.xLen)))

  val sqIdx = Vec(parameter.dispatchWidth, Output(new SqIdx(parameter.sqSize)))
}
...
class StoreQueue(val parameter: StoreQueueParameter)
  ...
  when (io.enq.fire) {
    for (i <- 0 until parameter.dispatchWidth) {
      val idx = sqEnqPVec(PopCount(enqValid.take(i)))
      val idxOH = sqEnqPOHVec(PopCount(enqValid.take(i)))
      io.sqIdx(i).idx  := idx
      io.sqIdx(i).flag := Mux(sqEnqPOH(parameter.sqNum - 1,
                                       parameter.sqNum - parameter.dispatchWidth).orR &&
                              idxOH(parameter.dispatchWidth - 1, 0).orR, !sqEnqFlag, sqEnqFlag)
      allocated(idx) := true.B
    }
  }

  when (io.wb.valid) {
    sqEntries(io.wb.bits.sqIdx).addr := io.wb.bits.addr
    writebacked(io.wb.bits.sqIdx) := io.wb.bits.dataValid
  }

  val sqEnqPOHNext = sqEnqPOHVec(PopCount(enqValid))
  when (io.enq.fire) {
    sqEnqPOH := sqEnqPOHNext
    when (sqEnqPOH(parameter.sqNum - 1,
                   parameter.sqNum - parameter.dispatchWidth).orR &&
          sqEnqPOHNext(parameter.dispatchWidth - 1, 0).orR) {
      sqEnqFlag := !sqEnqFlag
    }
  }
  .elsewhen (flush) {
